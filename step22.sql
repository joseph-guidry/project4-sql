...
SELECT HomeTeam, AwayTeam, concat( HomeTeamScore, '-', AwayTeamScore) AS "Score" 
	FROM 
		(SELECT GameID, name AS "HomeTeam", TeamScore AS "HomeTeamScore" 
		FROM winners 
		LEFT JOIN (teams)
		ON winners.team = teams.teamID
		ORDER BY GameID) AS t1
	JOIN
		(SELECT GameID, name AS "AwayTeam", TeamScore AS "AwayTeamScore" 
		FROM losers 
		LEFT JOIN (teams)
